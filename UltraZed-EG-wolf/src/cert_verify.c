...
int VerifyCert_Test(void)
{
    ...
    if ((cm = wolfSSL_CertManagerNew()) == NULL) {
        xil_printf("cert manager new failed\n");
        return -1;
    }

    ret = wolfSSL_CertManagerLoadCABuffer(cm, caCert, caSz, SSL_FILETYPE_PEM);
    if (ret != WOLFSSL_SUCCESS) {
        xil_printf("loading the ca chain failed\n");
    ...
    if (ret != WOLFSSL_SUCCESS) {
        xil_printf("could not verify certificate.\n");
        xil_printf("Error: (%d): %s\n", ret, wolfSSL_ERR_reason_error_string(ret));
        wolfSSL_CertManagerFree(cm);
        return -2;
    }

    xil_printf("Verification successful on cert1!\n");
    ...
    xil_printf("Verification successful on cert2!\n");
    xil_printf("------------------------------------------------------------\n\n");
